...
/**
 * Implementation of the Floodlight TopologyManager for CCBalancer service.
 *
...
public class TopologyManagerCCBalancer extends ServerResource implements 
        IFloodlightModule, ITopologyService, 
        IRoutingService, ILinkDiscoveryListener,
        IOFMessageListener, IHAListener, ICCBalancerListener {
	
    protected static Logger log = LoggerFactory.getLogger(TopologyManagerCCBalancer.class);
    ...
    public void init(FloodlightModuleContext context)
    	...
    	mpbalance = context.getServiceImpl(ICCBalancerService.class);
    ...
    protected boolean createNewInstance()  {
        ...
        }catch(IOException e){log.error("Error!");}
        return true;
    }
    ...
    */
	...
	//Update Static flow
	public void createNewFlowInstance() throws IOException {
     
		/*
		FileInputStream fstream = null;
        String name = null;
        String tempname = null;
        String src_switch = null;
        String dst_switch = null;
        String src_port;
        String flowName;
        String flowRevName;
        ...
        int size = 0;
        int counter = 0;
        NodePortTuple campo = null;
        
        try{
        	fstream = new FileInputStream("/home/simon/Dropbox/floodlight/floodlight/apps/circuitpusher/circuits.json");
        }catch (Exception e){
      log.error("Error! file Circuits.JSON not found!"); return;
      }
        ...
        Map<String, Object> newRowValues = new HashMap<>();
        Map<String, Object> newRevRowValues = new HashMap<>();
        strLine = br.readLine();
        
        while (strLine!= null)   {
        rowValues = StaticFlowEntries.jsonToStorageEntry(strLine);
        name = (String) rowValues.get("name");
        
        
        src_switch = (String) rowValues.get("switch_id");
        src_port = (String) rowValues.get("in_port");
        priority = Long.valueOf(rowValues.get("priority").toString());
        newRowValues.put("nw_src", rowValues.get("nw_src").toString());
        newRowValues.put("nw_dst", rowValues.get("nw_dst").toString());
        newRevRowValues.put("nw_dst", rowValues.get("nw_src").toString());
        newRevRowValues.put("nw_src", rowValues.get("nw_dst").toString());
        newRowValues.put("active", rowValues.get("active").toString());
        newRevRowValues.put("active", rowValues.get("active").toString());
        strLine = br.readLine();
        while (strLine != null)
        {
        	rowValues = StaticFlowEntries.jsonToStorageEntry(strLine);
        	tempname = (String) rowValues.get("name");
        	if (name.equals(tempname))
        		{
        			dst_switch = (String) rowValues.get("switch_id");
        			dst_port = (String) rowValues.get("tp_dst");
        			strLine = br.readLine();
        		}
        	else
        	{
        ...
        Route route = getRoute(HexString.toLong(src_switch), Short.valueOf(src_port), HexString.toLong(dst_switch),Short.valueOf(dst_port));
        ...
        size = switchPorts.size();
        for (counter = 0; counter<size ; counter++)
        {
        	campo = switchPorts.get(counter);
        	newRowValues.put("priority", String.valueOf(priority+5));
        	newRowValues.put("name", name+"temp");
        	newRowValues.put("switch_id", HexString.toHexString(campo.getNodeId()));
        	newRowValues.put("in_port", campo.getPortId());
        	newRevRowValues.put("priority", String.valueOf(priority+5));
        	newRevRowValues.put("name", name+"revtemp");
        	newRevRowValues.put("switch_id", HexString.toHexString(campo.getNodeId()));
        	newRevRowValues.put("actions", "output="+campo.getPortId());


        	
        	campo = switchPorts.get(++counter);
        	newRowValues.put("dl_type", "0x800");
        	newRowValues.put("actions", "output="+campo.getPortId());
        	newRevRowValues.put("dl_type", "0x800");
        	newRevRowValues.put("in_port", campo.getPortId());
        	
        	storageSource.insertRowAsync(StaticFlowEntryPusher.TABLE_NAME, newRowValues);
        	storageSource.insertRowAsync(StaticFlowEntryPusher.TABLE_NAME, newRevRowValues);
        	
        	newRowValues.put("name", name+"arptemp");
        	newRowValues.put("dl_type", "0x806");
        	newRevRowValues.put("name", name+"revarptemp");
        	newRevRowValues.put("dl_type", "0x806");
        	storageSource.insertRowAsync(StaticFlowEntryPusher.TABLE_NAME, newRowValues);
        	storageSource.insertRowAsync(StaticFlowEntryPusher.TABLE_NAME, newRevRowValues);

        	flowName = HexString.toHexString(campo.getNodeId()) + name + ".f";
        	flowRevName = HexString.toHexString(campo.getNodeId()) + name + "reverse.f";
        	storageSource.deleteRowAsync(StaticFlowEntryPusher.TABLE_NAME, flowName);
        	storageSource.deleteRowAsync(StaticFlowEntryPusher.TABLE_NAME, flowRevName);

        	

        	flowName = HexString.toHexString(campo.getNodeId()) + name + ".farp";
        	flowRevName = HexString.toHexString(campo.getNodeId()) + name + "reverse.farp";
        	storageSource.deleteRowAsync(StaticFlowEntryPusher.TABLE_NAME, flowName);
        	storageSource.deleteRowAsync(StaticFlowEntryPusher.TABLE_NAME, flowRevName);

        	
        	newRowValues.put("priority", String.valueOf(priority));
        	newRowValues.put("name", HexString.toHexString(campo.getNodeId())+ name+".f");
        	newRowValues.put("dl_type", "0x800");
        	newRevRowValues.put("priority", String.valueOf(priority));
        	newRevRowValues.put("name", HexString.toHexString(campo.getNodeId())+ name+"reverse.f");
        	newRevRowValues.put("dl_type", "0x800");
        	
        	storageSource.insertRowAsync(StaticFlowEntryPusher.TABLE_NAME, newRowValues);
        	storageSource.insertRowAsync(StaticFlowEntryPusher.TABLE_NAME, newRevRowValues);
        	
        	newRowValues.put("name", HexString.toHexString(campo.getNodeId())+ name+".farp");
        	newRowValues.put("dl_type", "0x806");
        	newRevRowValues.put("name", HexString.toHexString(campo.getNodeId())+ name+"reverse.farp");
        	newRevRowValues.put("dl_type", "0x806");
        	storageSource.insertRowAsync(StaticFlowEntryPusher.TABLE_NAME, newRowValues);
        	storageSource.insertRowAsync(StaticFlowEntryPusher.TABLE_NAME, newRevRowValues);
        	
        	flowName = name+"temp";
        	flowRevName = name+"revtemp";
        	storageSource.deleteRowAsync(StaticFlowEntryPusher.TABLE_NAME, flowName);
        	storageSource.deleteRowAsync(StaticFlowEntryPusher.TABLE_NAME, flowRevName);
        	
        	flowName = name+"arptemp";
        	flowRevName = name+"revarptemp";
        	storageSource.deleteRowAsync(StaticFlowEntryPusher.TABLE_NAME, flowName);
        	storageSource.deleteRowAsync(StaticFlowEntryPusher.TABLE_NAME, flowRevName);
...
*/
	...
	public void staticIpRoute(String srcSwitch, String srcPort, String dstSwitch, String dstPort, String IPSrc, String IPDst)
	{
        Map<String, Object> newRowValues = new HashMap<>();
        Map<String, Object> newRevRowValues = new HashMap<>();
        NodePortTuple campo = null;
        ...
        newRowValues.put("nw_src", IPSrc);
        newRowValues.put("nw_dst", IPDst);
        newRevRowValues.put("nw_dst", IPSrc);
        newRevRowValues.put("nw_src", IPDst);
        newRowValues.put("active", "true");
        newRevRowValues.put("active", "true");
        
        
        Route route = getRoute(Long.valueOf(srcSwitch), Short.valueOf(srcPort), Long.valueOf(dstSwitch),Short.valueOf(dstPort));
        log.error(route.toString());
        List<NodePortTuple> switchPorts = null;
        ...
        for (int counter = 0; counter<switchPorts.size() ; counter++)
        {
        	...
        	storageSource.deleteRowAsync(StaticFlowEntryPusher.TABLE_NAME, campo.getNodeId() + ":" + name + ".f");
        	storageSource.deleteRowAsync(StaticFlowEntryPusher.TABLE_NAME, campo.getNodeId() + ":" + name + ".farp");
        	storageSource.deleteRowAsync(StaticFlowEntryPusher.TABLE_NAME, campo.getNodeId() + ":" + revname + ".f");
        	storageSource.deleteRowAsync(StaticFlowEntryPusher.TABLE_NAME, campo.getNodeId() + ":" + revname + ".farp");
        	
        	newRowValues.put("priority", String.valueOf(30000));
        	newRevRowValues.put("priority", String.valueOf(30000));
        	
        	newRowValues.put("name", campo.getNodeId() + ":" + name + ".f");
        	newRevRowValues.put("name", campo.getNodeId() + ":" + revname + ".f");
        	...
        	newRowValues.put("name", campo.getNodeId() + ":" + name + ".farp");
        	newRevRowValues.put("name", campo.getNodeId() + ":" + revname + ".farp");
